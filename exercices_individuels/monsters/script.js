...
function askDamages(player) {
    ...
    do {
        playerPrompt = Number.parseInt(prompt(`Player ${player}, how many damages would you like to inflige ? (1-6)`)); // on demande à l'utilisateur
...
function play() {
    ...
    while (monsterPv > 0) { // ici on fait une boucle while car on sait combien de fois on va itérer (on l'occurence 50, car le monstre à 50 points de vie)
        ...
        if ((player = 1)) { // petite condition pour changer de joueur si le joueur est 1 alors le changer à deux car il vient de jouer
            player = 2;
        } else { // si ce n'est pas le joueur 1 ça veut dire que le joueur 2 vient de jouer donc repasser au joueur 1
            player = 1;
        }
        // player = 1 ? 2 : 1; version ternaire
        if (monsterPv <= 0) { // gérer la victoire et la mort du monstre
